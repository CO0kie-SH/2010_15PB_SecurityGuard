...
// F, G, H and I are basic MD5 functions.
    ...
    MD5::MD5(const std::string& input) :MD5(input.c_str(), (size_type)input.size())
    {
    }
    ...
    std::string MD5::hexdigest() const
    {
        ...
        char buf[33];   char* buff = buf;
        for (char i = 0; i < 16; i++, buff += 2)
            snprintf(buff, 3, "%02X", digestRaw[i]);
        //buf[32] = 0;

        return std::string(buf);
    }

    bool MD5::GetResult(char cBuff[33])
    {
        char* buff = cBuff;
        for (char i = 0; i < 16; i++, buff += 2)
            snprintf(buff, 3, "%02X", digestRaw[i]);
